...
public class DisplayOptionDataService : IDisplayOptionDataService
{
    ...
    private List<IdValuePair1> _sortedPlayerBadgeDisplayOptionList = [];
    ...
    public async Task InitializeAsync()
    {
        ...
        _sortedPlayerBadgeDisplayOptionList = Enumerable.Range(-1, 55).Select(x => new IdValuePair1 { Id = x, Value = x == -1 ? "不显示玩家等级和徽章" : $"{x}" }).ToList();
    }
    ...
    {
        return _sortedPlayerBadgeDisplayOptionList;
    }
}