...
public class GetRecentBattleHistoriesCommandHandler(ServerDbContext context) 
    : IRequestHandler<GetRecentBattleHistoriesCommand, List<BattleHistorySummary>>
{
    private readonly ServerDbContext _context = context;

    public Task<List<BattleHistorySummary>> Handle(GetRecentBattleHistoriesCommand request, CancellationToken cancellationToken)
    {
        ...
        var battleHistorySummaries = battleHistories
            .Select(battleHistory =>
            {
                ...
                var battleHistorySummary = new BattleHistorySummary()
                {
                    ...
                    BattleMode = battleHistory.BattleMode,
                ...
                // 临时用log来算出总的觉醒数..
                if (battleHistorySummary.BurstStats != null && battleHistorySummary.BurstStats.BurstCount == 0)
                {
                    battleHistorySummary.BurstStats.BurstCount = (uint)actionItems.Count(x => x.CardId == cardProfile.Id && x.PlayerName == cardProfile.UserName && x.ActionType == 3);
                }
