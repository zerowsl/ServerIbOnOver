...
public class UpdateBasicProfileCommandHandler : IRequestHandler<UpdateBasicProfileCommand, BasicResponse>
{
    ...
    public Task<BasicResponse> Handle(UpdateBasicProfileCommand request, CancellationToken cancellationToken)
    {
        ...
        var playerLv = _context.PlayerLevelDbSet.First(x => x.CardProfile == cardProfile);
        var playerBadge = _context.PlayerBadgeDbSet.FirstOrDefault(x => x.CardId == cardProfile.Id);
        ...
        playerLv.PrestigeId = basicProfile.PlayerBadge;
        playerProfile.OpenRecord = basicProfile.OpenRecord;

        playerBadge ??= new() { CardId = (uint)cardProfile.Id };
        playerBadge.BadgeId = basicProfile.PlayerBadge;
        if (playerBadge.Id <= 0) _context.PlayerBadgeDbSet.Add(playerBadge);
        else _context.PlayerBadgeDbSet.Update(playerBadge);
