...
public class ReleasedContentCommand : ILoadGameDataCommand
{
    ...
    public void Fill(Response.LoadGameData loadGameData)
    {
        ...
        var releasedNavis = Enumerable.Range(1, 60).Select(i => (uint)i).ToArray();
        var newMobileSuits = _config.GameConfigurations.NewMobileSuits ?? new[] { 329u };
		...
		loadGameData.MobileSuitBlocklists = [];
    }
}